...
# 这是你上上个回答里更科学的建议：flow-level split + flow 内 packet 配额
...
def build_and_save_flow_pool(flow_index, flow_pool_path: Path):
    by_app = defaultdict(list)
    for global_flow_key, meta in flow_index.items():
        by_app[meta["app_id"]].append((global_flow_key, meta))
...
# =====================================================
# 9. 从 flow_pool 切 train/valid/test
# =====================================================
...
# =====================================================
# 11. 第二阶段：按 split 重扫 pcap，导出 packet 到临时 jsonl
#     读取阶段也按 pcap 大小早停
# =====================================================
def build_flow_lookup(split_records):
    """
    split_records: train/valid/test 的 flow 元信息列表
    返回:
        lookup[(pcap_path, flow_key)] = meta
    其中 flow_key 在这里重建为字符串外的 tuple，靠 pcap_path + flow_id 对不上，
    所以改为 lookup by:
        lookup_by_pcap[pcap_path][flow_id]? 不行，第二次扫 pcap 时拿不到 flow_id。
    因此这里需要重新构造 global key:
        (pcap_path, proto, sip, sport, dip, dport)
    但 flow_pool 里没保存 flow_key。
    所以在写 flow_pool 时必须带 flow_key。
    """
    raise RuntimeError("Internal design error: flow_pool records must contain flow_key.")


# =====================================================
# 12. 为了第二阶段可查找，把 flow_key 也保存进 flow_pool
#     先补一个转换函数
# =====================================================
def rebuild_flow_pool_records_with_key(flow_index):
    """
    把 build_full_flow_index 的结果转换成可落盘的记录；
    记录里显式保存 global_flow_key 里的 flow_key，便于第二阶段重建查找。
    """
    records = []
...
def dump_packets_to_tmp(vpn_pcaps, nonvpn_pcaps, split_lookup):
    ...
    for idx, (pcap_path, vpn_label) in enumerate(jobs, 1):
        ...
        while True:
            ...
            # 关键：输出 dict，键用字符串，兼容 PacRep 原代码
...
def main():
    ...
    # ---------- 第三步：读取 split flow 文件，导出 packet ----------
