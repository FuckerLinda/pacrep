...
# =====================================================
# 5. 第一阶段：只建 flow 索引，不存所有 packet 文本
# =====================================================
def scan_one_pcap_build_flow_meta(pcap_path: Path, vpn_label: int, app_id: int, max_packets_per_pcap=None):
...
def build_flow_index(vpn_pcaps, nonvpn_pcaps):
    ...
    for idx, (pcap_path, vpn_label) in enumerate(jobs, 1):
        ...
        local_counts = scan_one_pcap_build_flow_meta(
            pcap_path=pcap_path,
            vpn_label=vpn_label,
            app_id=app_id,
...
# =====================================================
# 7. flow 切分：8:1:1 是总体目标；小类允许偏离，只保底
# =====================================================
...
# =====================================================
# 9. 第二阶段：按 split 重扫 pcap，导出 packet 到临时 jsonl
# =====================================================
def dump_packets_to_tmp(vpn_pcaps, nonvpn_pcaps, flow_index):
    ...
    for idx, (pcap_path, vpn_label) in enumerate(jobs, 1):
        ...
        print(f"[phase2 {idx}/{len(jobs)}] {pcap_path.name}: final_written={local_written}")
